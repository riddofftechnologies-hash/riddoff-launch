...
const EMPTY: FirestoreCourse & { id: string } = {
  id: "",
  title: "",
...
interface Props {
  initial?: FirestoreCourse & { id: string };
  onSave: (id: string, data: FirestoreCourse) => Promise<void>;
  onCancel: () => void;
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (imageFile) {
        ...
        imageUrl = await uploadImage(imageFile, `courses/${form.id.trim()}/cover.${ext}`);
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Row label="Title" required>
        <input value={form.title} onChange={(e) => set("title", e.target.value)} className={inp} placeholder="Course title" />
      </Row>
      <Row label="Category" required>
        <select value={form.category} onChange={(e) => set("category", e.target.value)} className={inp} title="Category">
          {CATEGORIES.map((c) => <option key={c}>{c}</option>)}
        </select>
      </Row>
      <Row label="Tagline">
        <input value={form.tagline} onChange={(e) => set("tagline", e.target.value)} className={inp} placeholder="One-line description" />
      </Row>
      <Row label="Short description">
        <input value={form.description} onChange={(e) => set("description", e.target.value)} className={inp} placeholder="2-3 sentence overview" />
      </Row>
      <Row label="Full description">
        <textarea rows={3} value={form.fullDescription} onChange={(e) => set("fullDescription", e.target.value)} className={inp} placeholder="Detailed description for the course page" />
      </Row>
      <div className="grid grid-cols-3 gap-3">
        <Row label="Hours">
          <input type="number" value={form.hours} onChange={(e) => set("hours", e.target.value)} className={inp} placeholder="12" />
        </Row>
        <Row label="Weeks">
          <input type="number" value={form.weeks} onChange={(e) => set("weeks", e.target.value)} className={inp} placeholder="4" />
        </Row>
        <Row label="Price (INR)">
          <input type="number" value={form.priceLow} onChange={(e) => set("priceLow", e.target.value)} className={inp} placeholder="1999" />
        </Row>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Row label="Enrolled count">
          <input type="number" value={form.enrolledCount} onChange={(e) => set("enrolledCount", e.target.value)} className={inp} placeholder="0" />
        </Row>
        <Row label="Review count">
          <input type="number" value={form.reviewCount} onChange={(e) => set("reviewCount", e.target.value)} className={inp} placeholder="0" />
        </Row>
      </div>
      ...
      <Row label="Cover Image">
        ...
        >
          ...
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImagePick} aria-label="Upload cover image" />
        </div>
        <input
          value={form.image}
      ...
      <Row label="Level">
        <select value={form.level} onChange={(e) => set("level", e.target.value as FirestoreCourse["level"])} className={inp} title="Level">
          <option>Beginner</option><option>Intermediate</option><option>Advanced</option>
        </select>
      </Row>
      <Row label="Outcomes (one per line)">
        <textarea rows={3} value={form.outcomes.join("\n")} onChange={(e) => handleArrayField("outcomes", e.target.value)} className={inp} placeholder="One outcome per line" />
      </Row>
      <Row label="Syllabus modules (one per line)">
        <textarea rows={4} value={form.syllabus.join("\n")} onChange={(e) => handleArrayField("syllabus", e.target.value)} className={inp} placeholder="One topic per line" />
      </Row>
